...
\begin{abstract}
	\bigskip
The goal of this thesis is to design and implement a new tool in the map-publicating platform Gisquick. The new tool focus on the management of spatio-temporal data. It provides simple user interface that enables map features filtration based on their time values. Filtering tool may be applied only for vector layers. In addiction, selection could be done automatically in order to make simple animations.
...
Second chapter describes the implementation of optimal solution on Gisquick platform. Filtering tool in the web client was developed in JavaScript programing language using Vue.js framework. The entire web client is being migrated into this framework at the moment. Another change is completed in Gisquick QGIS plugin. It allows set time layers during the publication process when attribute containing time values has to be identified for each time layer.
