...
%% ML: vyhni se sklonovani, napr. Predstaveni platformy Gisquick
%% DT : opraveno
\section{Představení platformy Gisquick}

%% ML: daty->zdrojovym kodem (nebo co jsi mel na mysli?)
%% DT : zdrojový kód
...
Gisquick.
%% ML: vyhni se zavorce, vetu prepis, je divna
%% DT: prepsano
...
%% ML: jsou popsany v nasledujici kapitole.
%% DT: preformuovano
...
\begin{itemize}
%% ML: prvni veta je nejasna, jaky projekt? prepsat
%% DT: prepsano
...
%% ML: ulozeny -> upraveny (?) Vstupem je take QGIS projekt. Mluvis
%% najednou o formatu, klidne bych se drzel ``QGIS projekt''
%% DT: opraveno na "upravený QGIS projekt"
...
%% ML: prvni veta moc nedava smysl, prepis
%% ML: muzes uvest odkaz na kapitolu v casti I
%% DT: opraveno, doplneno
...
%% ML: cele?
%% DT: odstraneno
%% ML: interagovat -> cestina
%% ML: orientovat? zkus prepsat cela veta zni divne
%% DT: preformulovano
...
%% ML: rozdelit na dve podkapitoly: UI weboveho klienta a zasuvneho modulu
\begin{figure}[h!]
\centering
...
%% ML: uved konkretni odkaz na kapitolu
%% DT: doplneno
...
%% ML: Veta zacinajici na ``Jeho hlavni pouziti ...'' nezni moc pekne...
%% DT: preformulovano 
...
%% ML: vystupy? mozna soubory vytvarene Gisquick pluginem?
%% DT: doplneno
V kapitole \ref{sssec:gisquick-komponenty} jsou zmíněny soubory vytvářené
Gisquick pluginen. 
...
%% ML: umoznuje nesedi, bude umoznovat anebo umoznujici
%% DT: umoznujici
umožňující definovat všechny potřebné parametry sloužící ve
...
%% ML: napis proc, rastrova data jsou nad ramec zadani prace
%% DT: doplneno
...
%% ML: vsechny/kazde zni divne
%% DT: opraveno
%% ML: co to znamena? pro nastaveni (prazdny retezec), zkus prepsat,
%% at je to jasnejsi
%% DT:preformulovano
...
%% ML: klientovi, zkus ten odstavec jeste jednou projit a procistit
%% DT: opraveno
...
%% ML: rozdelit souveti?
%% DT: preformulovano
V předposledním kroku publikace se lze přesvědčit, zad-li výpočet 
...
%% ML: vypocet ceho, vysvetlit (mas to vysvetleno dale v textu), tady
%% by stacilo jednim slovem
%% DT: prepsano
...
%% ML: prazdny -> neni zadan
%% DT: oraveno 
podmínky. Jejich časový atribut musí existovat a musí být zadán. 
%% ML: Rastry nejsou soucasti zadani, ta veta je zavadejici.
%% DT: preformulovano
...
%% ML: itemize -> enumerate ?
%% DT: enumerate je lepsi
\begin{enumerate}
...
vhodném pro následnou filtraci mapových prvků. Vhodný časový formát nesmí 
obsahovat speciální znak a dále musí být pouze ve formátu 
začínajícím rokem, nebo hodinou např. \uv{YYYY-MM-DD}, nebo \uv{HH-mm}. 
Vhodný časový formát dále nesní obsahovat speciální znak. 
Například při použití data v textovém formátu
\uv{YYYY-MM-DDTHH:mm:SS}, kdy je použit znak 'T' oddělující
datum a čas, nelze textový řetězec v parametru
%% ML: za viz se nepise carka, zkontroluj dale v textu
%% DT: zkontrolovano, opraveno
...
%% ML: davej si pozor na predlozky na konci radky, tady doplneno ~,
%% zkrontroluj to dale v textu
%% DT: zkontrolovano
...
%% ML: vysvetlit unix time - poznamka pod carou
%% DT: doplneno
...
validace, která spoléhá na fakt, že vstupní data obsahují pouze jeden časový 
...
%% ML: prvni vetu prespis, nezni cesky
%% DT: preformuovano
...
%% ML: pro uvozovky pouzivej \uv{}
%% DT: ok
vrstvy. Dále je zde možnost \uv{All visible layers}, která uživateli
...
časovým parametrem. Při výběru možnosti \uv{All visible layers} je
...
%% ML: je nutna? I tak zkus prepsat
%% DT: prepsano
...
%% ML: kde je dokumentace?
%% DT: dokumentace jeste nebyla vytvorena. Doplneno o referenci
...
%% ML: nazev kapitoly zni divne, zkus preformulovat
%% DT: preformulovano
